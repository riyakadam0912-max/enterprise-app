...
export default function TeamAttendance() {
  ...
  return <ScrollView refreshControl={<RefreshControl refreshing={query.isFetching} onRefresh={() => query.refetch()} />} contentContainerStyle={{ padding: 20, backgroundColor: '#f8fafc', flexGrow: 1 }}>
    ...
    <View style={{ backgroundColor: '#fff', borderRadius: 14, padding: 16, marginTop: 18 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}><CalendarCheck color="#059669" size={21} /><Text style={{ color: '#172033', fontSize: 18, fontWeight: '800' }}>People today</Text></View>{rows.length === 0 ? <Text style={{ color: '#64748b' }}>No attendance records in this scope.</Text> : rows.map((row, index) => { const employeeUserId = Number((row.employee as any)?.userId ?? (row.employee as any)?.user?.id ?? row.employeeId ?? 0) || undefined; return <View key={`${row.employeeId ?? row.id ?? 'row'}-${index}`} style={{ borderTopWidth: index === 0 ? 0 : 1, borderTopColor: '#e2e8f0', paddingVertical: 12 }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}><View style={{ flex: 1 }}><UserIdentity userId={employeeUserId} name={row.employee?.name ?? `Employee ${row.employeeId ?? ''}`} subtitle={row.employee?.department ?? 'Unassigned department'} size="md" /></View><Text style={{ color: statusColor[row.status] ?? '#475569', fontWeight: '800' }}>{row.status.replaceAll('_', ' ')}</Text></View>{row.employee?.designation ? <Text style={{ color: '#64748b', marginTop: 6 }}>{row.employee.designation}</Text> : null}<Text style={{ color: '#64748b', marginTop: 6 }}>In: {row.checkIn ? new Date(row.checkIn).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) : 'Not recorded'} · Out: {row.checkOut ? new Date(row.checkOut).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) : 'Not recorded'}</Text>{row.lateMinutes > 0 || row.overtimeHours > 0 || (row.shortfallHours ?? 0) > 0 ? <Text style={{ color: '#b45309', marginTop: 5 }}>Late {row.lateMinutes}m · Overtime {row.overtimeHours}h · Shortfall {row.shortfallHours ?? 0}h</Text> : null}</View>; })}</View>
  </ScrollView>;
}