...
export type Task = { id: number; taskName: string; description?: string | null; links?: string | null; driveLink?: string | null; status?: string; priority?: string; assignee?: string | null; assignedToUserId?: number | null; assignedToId?: number | null; dueDate?: string | null; submissionLink?: string | null; submissionNotes?: string | null; reviewComment?: string | null };
export type TaskMessage = { id: string; taskId: number; senderId: number; content: string; createdAt: string; sender: { id: number; name: string; email: string } };
...
export async function taskMessages(id: number) { return unwrap<TaskMessage[]>((await api.get(`/tasks/${id}/messages`)).data); }
export async function sendTaskMessage(id: number, content: string) { return unwrap<TaskMessage>((await api.post(`/tasks/${id}/messages`, { content })).data); }
