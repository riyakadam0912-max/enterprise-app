...
export interface DashboardSummaryPayload {
  ...
  summaryCards: {
    ...
    presentToday: unknown;
    monthlyPayrollCost: unknown;
...
export class ReportsAnalyticsService implements OnModuleInit, OnModuleDestroy {
  ...
  async getAttendanceReport(
    ...
    return this.withCache('attendance', user, filters, async () => {
      ...
      const where = this.attendanceWhere(
        dateRange,
        scope,
        filters.department,
        filters.role,
      ...
      const [totalDays, presentDays, absentDays, lateCount, overtime] =
        await Promise.all([
  ...
  async getDailyAttendanceBreakdown(
    user: ReportsUserContext,
    filters: ReportsQueryFilters,
  ) {
  ...
  async getDepartmentAttendance(
    ...
    return this.withCache('attendance-departments', user, filters, async () => {
      ...
      const map = new Map<
        string,
        { total: number; present: number; absent: number; lateCount: number }
      >();

      for (const row of rows) {
        const department = row.employee?.department ?? 'Unassigned';
        const prev = map.get(department) ?? {
          total: 0,
          present: 0,
          absent: 0,
          lateCount: 0,
    ...
    return this.withCache('payroll-trends', user, filters, async () => {
      ...
      const results = await Promise.all(
        months.map(async (monthPoint) => {
          ...
          const where = this.payrollWhere(
            { start: from, end: to, monthLabel: monthPoint.label },
            scope,
            filters.department,
            filters.role,
          );
  ...
  async getTurnoverSummary(
    ...
    return this.withCache('turnover', user, filters, async () => {
      const scope = await this.resolveScope(user, filters);
      const dateRange = this.resolveDateRange(filters);
      const employeeWhere = this.employeeWhere(
        scope,
        filters.department,
        filters.role,
      );
      ...
      const averageEmployees =
        (openingEmployees + closingEmployees) / 2 || totalEmployees || 1;
      const attritionRate = Number(
        ((resignations / averageEmployees) * 100).toFixed(2),
      );
    ...
    return this.withCache('performance', user, filters, async () => {
      ...
      const performerRows = groupedRatings
        .map((item) => ({
          ...
          department:
            employeeMap.get(item.employeeId)?.department ?? 'Unassigned',
        }))
  ...
  async getDashboardSummary(
    ...
    if (cached && Date.now() - cached.cachedAt < 5 * 60 * 1000) {
      return cached.payload as DashboardSummaryPayload;
    }
  ...
  private async resolveScope(
    ...
    if (user.role === 'EMPLOYEE') {
      if (!user.employeeId) {
        throw new ForbiddenException(
          'Employee mapping missing for current user',
        );
      }
      if (requestedEmployeeId && requestedEmployeeId !== user.employeeId) {
        throw new ForbiddenException(
          'Employees can only access their own analytics',
        );
      }
      return {
        role: user.role,
    ...
    if (requestedEmployeeId) {
      if (!distinctTeamEmployeeIds.includes(requestedEmployeeId)) {
        throw new ForbiddenException(
          'Managers can only access their team analytics',
        );
      }
      return {
        role: user.role,
  ...
  private async precomputeCurrentMonth() {
    ...
    const payload = await this.getDashboard(adminContext, {
      month,
      limit: '10',
      page: '1',
    });
