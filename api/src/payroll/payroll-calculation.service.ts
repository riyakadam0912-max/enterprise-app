...
interface PayrollStructureLike {
  basic?: number | null;
  hra?: number | null;
  allowances?: number | null;
  overtimeRatePerHour?: number | null;
  tds?: number | null;
  allowedLateMarks?: number | null;
  lateMarkPenalty?: number | null;
}
...
interface PayrollCalculationInput {
  ...
  month: number;
  year: number;
  salaryStructure: PayrollStructureLike;
  attendanceData?: {
    presentDays: number;
    absentDays: number;
    paidLeaves: number;
...
export class PayrollCalculationService {
  ...
  private calculateEarnings(
    ...
    const totalEarnings =
      basic + hra + allowances + overtime + reimbursements + bonus;
  ...
  private calculatePF(basicSalary: number, isEmployee: boolean): number {
    const rate = isEmployee
      ? this.config.pfEmployeeRate
      : this.config.pfEmployerRate;
  ...
  private calculateProfessionalTax(grossSalary: number): number {
    const applicableSlab = this.ptSlabs.find(
      (slab) => grossSalary >= slab.minIncome && grossSalary < slab.maxIncome,
    );
  ...
  calculateMonthlyTDS(
    annualSalary: number,
    taxDeclaration: Prisma.TaxDeclarationGetPayload<object> | null,
  ...
  private calculateAnnualTaxableIncome(
    annualSalary: number,
    taxDeclaration: Prisma.TaxDeclarationGetPayload<object> | null = null,
  ): number {
    const {
      investment80C = 0,
      investment80D = 0,
    ...
    const deductions =
      investment80C + investment80D + investment80CCD + hraExemption;
  ...
  private calculateTaxableIncome(
    grossEarnings: number,
    deductions: PayrollCalculationResult['deductions'],
  ): number {
    return Math.max(
      0,
      grossEarnings -
